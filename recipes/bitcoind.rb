...
%w[build-essential libtool autotools-dev autoconf pkg-config libssl-dev libboost-all-dev libcurl4-openssl-dev libevent-dev].each do |pkg|
  package pkg do
    action :install
  end
end
...
git "#{node['cryptonet-opsworks']['bitcoind']['src_prefix']}/bitcoin" do
  repository node['cryptonet-opsworks']['bitcoind']['repo']
